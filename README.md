...
- `/exportcode [floorsCSV] [name]` — экспорт этажей кода в `exportcode_*.json` (только блоки+таблички).
- `/codeselector` — выдать Code Selector (RMB toggle строку, LMB очистить, F закончить).
- `/exportselected [name]` — экспорт только выбранных строк в `exportselected_*.json`.
